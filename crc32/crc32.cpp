/*
  ...
  This entire implementation is taken from:
...
static constexpr uint64_t merger_lut_[] = {
  ...
  0 // This is a just-in-case entry to prevent
    // a possible falling outside the table address range.
};
...
// {
//     printf("static constexpr uint64_t merger_lut_[] = {\n");
...
#define PTR_(mem,b) reinterpret_cast<uint ## b ## _t*>(mem)
#define PTR8(mem) PTR_(mem, 8)
#define PTR64(mem) PTR_(mem, 64)

#define CRC_ITER_(i) case i:\
crcA = _mm_crc32_u64(crcA, *PTR64(pA - ((i)<<3)));\
crcB = _mm_crc32_u64(crcB, *PTR64(pB - ((i)<<3)));\
crcC = _mm_crc32_u64(crcC, *PTR64(pC - ((i)<<3)));

#define X0(n) CRC_ITER_(n);
...
#define SWITCH_CRC_ITERS_256_TO_2(n) switch(n) {\
X0(256) X1(254) X2(250) X3(242) X4(226) X5(194) X6(130) X7(2)}

crc32_t crc32::update(const void* buf, size_t size) {
  constexpr size_t LEAF_SIZE = 6 * 24; // must be >= 24 for Intel

  using ptr_t = uint64_t;
  auto pA = ptr_t(buf);
  static_assert(sizeof(pA) >= sizeof(buf));

  auto crcA = static_cast<uint64_t>(~_c);
  for (auto toAlign = ~pA & 7; toAlign && size; ++pA, --size, --toAlign)
    crcA = _mm_crc32_u8((uint32_t)crcA, *PTR8(pA));

  while (size >= LEAF_SIZE) {
    const uint32_t n = size < 256 * 24
      ? size * 2731 >> 16 // equivalent of size / 24, but faster
      : 256;
    ...
    SWITCH_CRC_ITERS_256_TO_2(n);

    crcA = _mm_crc32_u64(crcA, *PTR64(pA - 8));
    crcB = _mm_crc32_u64(crcB, *PTR64(pB - 8));
    const __m128i vK = _mm_cvtepu32_epi64(
      _mm_loadu_si128(reinterpret_cast<const __m128i*>(&merger_lut_[n - 1])));
    const __m128i vA = _mm_clmulepi64_si128(_mm_cvtsi64_si128(crcA), vK, 0x00);
    const __m128i vB = _mm_clmulepi64_si128(_mm_cvtsi64_si128(crcB), vK, 0x10);
    crcA = _mm_crc32_u64(crcC,
      _mm_cvtsi128_si64(_mm_xor_si128(vA, vB)) ^ *PTR64(pC - 8));

    size -= 24 * n;
    pA = pC;
  }

  for (; size >= 8; size -= 8, pA += 8)
    crcA = _mm_crc32_u64(crcA, *PTR64(pA));

  for (; size; --size, ++pA)
    crcA = _mm_crc32_u8((uint32_t)crcA, *PTR8(pA));

  return _c = ~static_cast<crc32_t>(crcA);
}
